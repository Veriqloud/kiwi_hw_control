...
def Shift_Unit(j,party):
	...
	times_ref_click1=[]
	if party == 'alice':
		int_click_gated = np.loadtxt("data/tdc/pm_a_shift_"+str(j+1)+".txt",usecols=(2,3,4),unpack=True, dtype=np.int64)
	elif party == 'bob':
		int_click_gated = np.loadtxt("data/tdc/pm_b_shift_"+str(j+1)+".txt",usecols=(2,3,4),unpack=True, dtype=np.int64)
