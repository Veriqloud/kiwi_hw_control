...
int main(int argc, char *argv[]){
    if (argc < 3){
        printf("usage:\n tdc_bin2txt inputfile.bin outputfile.txt\n");
        return 0;
    }
    ...
    unsigned int q_pos = 0;
    
    int count = 0;
    while (ret == 16){
        time = 0;
        time = (uint32_t) buf[0] + ((uint32_t) (buf[1] & 0x3f) << 8);
        idx = 0;
        idx = ((uint32_t) (buf[1]&0xc0)>>6) + ((uint32_t) (buf[2]&0x3) <<2);
        gc.value = 0;
        gc.b[0] = buf[4];
        gc.b[1] = buf[5];
        gc.b[2] = buf[6];
        ...
        gc.b[4] = buf[8];
        gc.b[5] = buf[9];
        result = (((int) buf[10]) & 0x2)>>1;
	q_pos = ((int) buf[10]) & 0x1;
        fprintf(ftxt, "%d\t%d\t%lu\t%d\t%d\n", idx, time, gc.value, result, q_pos);
        //printf("time %d\n", time);
        //printf("idx %d\n", idx);
        //printf("gc %lu\n", gc.value);
        //for (int i=0; i<16; i++){
