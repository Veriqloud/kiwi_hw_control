...
while True:
    ...
    #                t['insert_zeros'] = 'off'
     ...
     #                   diff = ctl.diff_counts()   
                ...
                raise  # let the outer handler report the disconnect
            except Exception as e:
                # A ctl.* call blew up mid-command (bad fit, div-by-zero, missing
                # file, ...). Previously this propagated past the accept() loop and
                # killed the whole hws_bob process, forcing a 10s systemd restart
                # (RestartSec=10) that Alice would ride out via its own Bob-link
                # reconnect logic anyway. Instead, drop just this connection: closing
                # it makes Alice's next send/recv fail with ConnectionError, which
                # hws_alice.py already handles by reconnecting to Bob.
                print(colored(f'error executing command {command}: {e}', 'red', force_color=True))
